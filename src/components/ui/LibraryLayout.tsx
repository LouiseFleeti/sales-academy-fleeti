      ...
      <aside className="w-64 shrink-0 border-r border-gray-100 bg-white flex flex-col">
        ...
        <div className="px-4 pt-5 pb-3">
          <div className="flex items-center gap-2 mb-3">
            <span style={{ color: "#3979C1" }}>{icon}</span>
            <h1 className="font-bold text-sm text-gray-900">{title}</h1>
            <span
              className="ml-auto text-xs font-semibold px-2 py-0.5 rounded-full"
              style={{ background: "#E8F2FD", color: "#3979C1" }}
            >
              {filtered.length}
            </span>
          </div>
                ...
                return (
                  <li key={item.id}>
                    <button
                      ...
                      style={isActive ? { background: "#E8F2FD" } : {}}
                    >
                      {/* Active indicator */}
                      {isActive && (
                        <span
                      ...
                      <p
                        ...
                        style={{ color: isActive ? "#3979C1" : "#1a1a1a" }}
                      >
                        {item.name}
                      </p>
                      {item.subtitle && (
        ...
        {selectedId ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400">
            <div
              ...
              style={{ background: "#E8F2FD" }}
            >
              <svg className="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="#3979C1" strokeWidth="1.5" opacity="0.5">
